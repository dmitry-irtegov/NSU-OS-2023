...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt(argc, argv, options)) != -1) {
        ...
        case 'c':
            ...
            break;
        case 'C':
            if (sizeof(new_core_size) == sizeof(long))
            {
                new_core_size = (rlim_t)atol(optarg);
            }
            else if (sizeof(new_core_size) == sizeof(long long))
            {
                new_core_size = (rlim_t)atoll(optarg);
            }
            else {
                new_core_size = (rlim_t)atol(optarg);
            }
            ...
            printf("The size of the core-file has been successfully changed to %lu bytes\n", new_rlim.rlim_cur);
            break;
        case 'd':
            cwd = getcwd(NULL, 256);
            if (cwd == NULL) {
