...
{
	errno = 0;
	long long new_soft_limit = strtoll(optarg, NULL, 10);
	if (errno != 0)
	{
		perror("Can not convert -U argument to long long type\n\n");
		return;
	}
	...
	new_u_limit.rlim_cur = (rlim_t)new_soft_limit;
...
static void handle_C()
{
	...
	if (errno != 0)
	{
		perror("Can not convert -C argument to long long type\n\n");
		return;
	}
	...
	new_core_limit.rlim_cur = (rlim_t)new_soft_limit;
